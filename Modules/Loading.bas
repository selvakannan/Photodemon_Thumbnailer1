...
Public Function LoadFileAsNewImage1(ByRef srcFile As String, Optional ByVal suggestedFilename As String = vbNullString, Optional ByVal addToRecentFiles As Boolean = True, Optional ByVal suspendWarnings As Boolean = False, Optional ByVal handleUIDisabling As Boolean = True, Optional ByVal overrideParameters As String = vbNullString) As Boolean
    ...
    If loadSuccessful And (targetDIB.GetDIBWidth > 0) And (targetDIB.GetDIBHeight > 0) And (Not targetImage Is Nothing) Then
        ...
        '*************************************************************************************************************************************
        ' If the loaded image was in KGX format (KannaGrafix's internal format), skip a number of additional processing steps.
        '*************************************************************************************************************************************
        ...
        '/End specialized handling for non-KGX files
        End If
    ...
    LoadFileAsNewImage1 = loadSuccessful And (Not targetImage Is Nothing)
    
    'Activate the new image (if loading was successful) and exit
    If LoadFileAsNewImage1 Then
    ...
    Else
        If (Macros.GetMacroStatus <> MacroBATCH) And (Not suspendWarnings) And (freeImage_Return <> PD_FAILURE_USER_CANCELED) And (Not userCanceledImportDialog) Then
           ...
           ' PDMsgBox "Unfortunately, KannaGrafix was unable to load the following image:" & vbCrLf & vbCrLf & "%1" & vbCrLf & vbCrLf & "Please use another program to save this image in a generic format (such as JPEG or PNG) before loading it.  Thanks!", vbExclamation Or vbOKOnly, "Image import failed", srcFile
        End If
    End If
